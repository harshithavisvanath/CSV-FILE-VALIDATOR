...
describe('<Files />', () => {
  ...
  it('allows us to set the "accepts" prop', () => {
    const wrapper = mount(<Files accepts={['.csv']} />)
    expect(wrapper.props().accepts[0]).to.equal('.csv')
  ...
  it('allows us to set the "maxFiles" prop', () => {
    const wrapper = mount(<Files maxFiles={1} />)
    expect(wrapper.props().maxFiles).to.equal(1)
  })
  ...
  it('allows us to set the "minFileSize" prop', () => {
    const wrapper = mount(<Files minFileSize={1} />)
    expect(wrapper.props().minFileSize).to.equal(1)
  })
